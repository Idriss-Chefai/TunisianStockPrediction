...
@Component({
  selector: 'app-live-stock-chart',
  standalone: true,
  imports: [],
  templateUrl: './live-stock-chart.component.html',
  styleUrl: './live-stock-chart.component.css'
})
export class LiveStockChartComponent {
  ...
  ngAfterViewInit() {
    ...
    setInterval(function () {
      ...
      if (valueSeries) {
        ...
        if (lastDataObject) {
          let previousDate = lastDataObject["Date"] as number;
          let previousValue = lastDataObject["Close"] as number;
    
          value = am5.math.round(previousValue + (Math.random() < 0.5 ? 1 : -1) * Math.random() * 2, 2);
    
          let high = lastDataObject["High"] as number;
          let low = lastDataObject["Low"] as number;
          let open = lastDataObject["Open"] as number;
