...
import { Stock } from '../stock.model'; // Import Stock interface
...
export class StockDetailComponent implements OnInit {
  stock: Stock | undefined; // Initialize stock object

  constructor(private route: ActivatedRoute, private router: Router) { }

  ngOnInit(): void {
    const stockName = this.route.snapshot.paramMap.get('name'); // Get stock name from route parameters
    console.log(stockName)
    if (stockName) {
      this.loadStockDetails(stockName); // Load stock details based on the name
    } else {
      // Handle the case where the 'name' parameter is not available
    }
  }

  loadStockDetails(stockName: string): void {
    ...
    // Find the stock with matching name
    this.stock = stocks.find(stock => stock.name === stockName);
  }
